...
class PDF extends FPDF
{
   ...
   function Header()
   {
      ...
      $this->Cell(35, 10, utf8_decode('CONTRASEÑA'), 1, 0, 'C', 1);
      $this->Cell(40, 10, utf8_decode('ROL'), 1, 1, 'C', 1);
   }
...
$data = new conexion();
   ...
   $resul = $conexion->prepare("SELECT * FROM t_usuario WHERE 1");
   $resul->execute();
...
for($i=0;$i<count($data);$i++){
   ...
   $pdf->Cell(20,10,$data[$i]['id_usuario'],1,0);
   $pdf->Cell(35,10,$data[$i]['nombre_usu'],1,0);
   $pdf->Cell(35,10,$data[$i]['password_usu'],1,0);
   if ($data[$i]['rol'] === 1) $rol = "Administrador"; else $rol = "Vendedor";
   $pdf->Cell(40,10,$rol,1,1);
